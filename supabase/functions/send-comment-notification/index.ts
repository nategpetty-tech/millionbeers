...
Deno.serve(async (request) => {
  ...
  if (!body.commentId || !body.checkInId) return json({ error: "commentId and checkInId are required." }, 400);
  ...
  const { data: comment, error: commentError } = await serviceClient
    ...
    .select("id,check_in_id,user_id,body")
    .eq("id", body.commentId)
    .eq("check_in_id", body.checkInId)
    .maybeSingle();
  if (commentError) return json({ error: "Could not load comment." }, 500);
  const commentRow = comment as CommentRow | null;
  if (!commentRow || commentRow.user_id !== user.id) return json({ ok: true, sent: 0 });
  ...
  const messages = tokens.map((to) => ({
    to,
    title: "Pintly",
    body: `${actorName} commented on your beer log${beerPhrase(row.beer_name)}`,
    sound: "default",
